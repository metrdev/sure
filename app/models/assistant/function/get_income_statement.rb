class Assistant::Function::GetIncomeStatement < Assistant::Function
  ...
  def call(params = {})
    ...
    statement = IncomeStatement.new(
      family,
      user: user,
      transactions_scope: Transaction.reportable_by(user),
...

    {
      ...
      insights: get_insights(income_data, expense_data, statement)
    }
  end
  ...
  private
    ...
    def get_insights(income_data, expense_data, statement)
      net_income = income_data.total - expense_data.total
      savings_rate = calculate_savings_rate(income_data.total, expense_data.total)
      median_monthly_income = statement.median_income
      median_monthly_expenses = statement.median_expense
      avg_monthly_expenses = statement.avg_expense
